...
To simplify further framework testing, development and extension spacial care was applied to design the class hierarchy. The class diagram is is presented in the Figure \ref{fig:class-diagam}.
...
\begin{itemize}
	...
	\item \emph{updateGrid()} - computes the cells domain, on which calculation of SDF will be performed.
	\item \emph{updateLevelSet()} - updates hash tables with values, required to compute level set
	\item \emph{getSDFvalue} - calculates resulting SDF value for a given MC cell vertex
\end{itemize}

\emph{MarchingCubes} class reimplements interface method \emph{generateMesh()} composing previously described functions as described in Algorithm \ref{alg:generateMesh}.
