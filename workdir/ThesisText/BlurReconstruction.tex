...
\end{algorithm}
The advantage of the static MC grid is that it has a static displacement of all grid vertices, thus neighborhood can be efficiently computed. However, as it appeared, using full neighborhood within the $kernelSize$ is not sufficient for blur algorithm in the application to surface reconstruction. As soon as the aim of the smoothing is to remove bumps in the flat surface areas it is more practical to take into account level set vertices, that are in the neighborhood of the fluid surface. Thus, for each vertex of the the level set gradient of the level set can be computed, and the gradient vector is a normal to the fluid surface \cite{LevelSetMethods}. Thus for the flat surface areas it is convenient to blur level set vertex value w.r.t. vertices, which resides in a tangential direction to the surface normal.
...
\begin{figure}[H]
 ...
 \end{figure}
At the Figure \ref{fig:expected_fs} expected fluid surface reconstruction in flat area is displayed. Dots of different colors are the MC grid vertices. Colors represents their level set value. In the perfect case level set ISO-lines (in the 2D example) forms perfect lines, thus MC grid vertices will receive similar values along the level set line. However, due to irregular displacement of fluid particles surface is usually reconstructed as shown in Figure \ref{fig:recon_fs}. In case if we apply full kernel size (see Figure \ref{fig:full_ks}) we will pick a level set values from different iso-surface levels, which will change level set values unpredictably. The better approach is to pick a kernel frame, so that only a level set values of near surface MC vertices will be used for blur operation (Figure \ref{fig:partial_ks}). This way blur operation will smooth out iso-surface lines of the level set in flat regions and results in a smoother surface. Especially the influence of the kernel depth can be seen in a thin areas, where a kernel size is larger than thickness of a fluid surface (see Figure \textcolor{red}{TODO: add figure}).

Example can be seen from the Figure \textcolor{red}{TODO: add example of kernel depth in real fluid domain}
